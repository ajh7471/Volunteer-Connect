...
// =========================================================
// SECTION 21: Bulk Create Shift — Input Contract Validation
// =========================================================
console.log("\n=== Section 21: bulkCreateShifts Input Contracts ===")

function validateBulkCreateInput(opts) {
  var errors = []
  if (!opts.slot || !["AM","MID","PM"].includes(opts.slot)) errors.push("invalid slot")
  if (!opts.startDate) errors.push("startDate required")
  if (!opts.endDate) errors.push("endDate required")
  if (opts.startDate && opts.endDate && opts.startDate > opts.endDate) errors.push("startDate must be <= endDate")
  if (!opts.daysOfWeek || opts.daysOfWeek.length === 0) errors.push("daysOfWeek must have at least one day")
  if (!opts.capacity || opts.capacity < 1) errors.push("capacity must be >= 1")
  if (!opts.startTime) errors.push("startTime required")
  if (!opts.endTime) errors.push("endTime required")
  if (opts.startTime && opts.endTime && opts.startTime >= opts.endTime) errors.push("startTime must be < endTime")
  return errors
}

// Valid input
var validBulk = { slot:"AM", startDate:"2026-04-01", endDate:"2026-04-30", daysOfWeek:[1,2,3,4,5], capacity:3, startTime:"09:00", endTime:"12:00" }
assertEqual(validateBulkCreateInput(validBulk).length, 0, "bulkCreate: valid input passes")
// Invalid slot
assert(validateBulkCreateInput({ ...validBulk, slot:"NIGHT" }).includes("invalid slot"), "bulkCreate: invalid slot rejected")
// Date range inverted
assert(validateBulkCreateInput({ ...validBulk, startDate:"2026-05-01", endDate:"2026-04-01" }).includes("startDate must be <= endDate"), "bulkCreate: inverted dates rejected")
// No days selected
assert(validateBulkCreateInput({ ...validBulk, daysOfWeek:[] }).includes("daysOfWeek must have at least one day"), "bulkCreate: empty daysOfWeek rejected")
// Zero capacity
assert(validateBulkCreateInput({ ...validBulk, capacity:0 }).includes("capacity must be >= 1"), "bulkCreate: zero capacity rejected")
// Time inverted
assert(validateBulkCreateInput({ ...validBulk, startTime:"12:00", endTime:"09:00" }).includes("startTime must be < endTime"), "bulkCreate: inverted times rejected")

// =========================================================
// SECTION 22: Bulk Delete Shift — Input Contract Validation
// =========================================================
console.log("\n=== Section 22: bulkDeleteShifts Input Contracts ===")

function validateBulkDeleteInput(opts) {
  var errors = []
  if (!opts.startDate) errors.push("startDate required")
  if (!opts.endDate) errors.push("endDate required")
  if (opts.startDate && opts.endDate && opts.startDate > opts.endDate) errors.push("startDate must be <= endDate")
  if (opts.slot && !["AM","MID","PM"].includes(opts.slot)) errors.push("invalid slot filter")
  return errors
}

var validDelete = { startDate:"2026-04-01", endDate:"2026-04-30" }
assertEqual(validateBulkDeleteInput(validDelete).length, 0, "bulkDelete: valid (no slot filter) passes")
assertEqual(validateBulkDeleteInput({ ...validDelete, slot:"AM" }).length, 0, "bulkDelete: valid slot filter passes")
assert(validateBulkDeleteInput({ ...validDelete, slot:"BOGUS" }).includes("invalid slot filter"), "bulkDelete: invalid slot rejected")
assert(validateBulkDeleteInput({ startDate:"2026-05-01", endDate:"2026-04-01" }).includes("startDate must be <= endDate"), "bulkDelete: inverted range rejected")
assert(validateBulkDeleteInput({ endDate:"2026-04-30" }).includes("startDate required"), "bulkDelete: missing startDate rejected")

// =========================================================
// SECTION 23: Bulk Update Capacity — Input Contract Validation
// =========================================================
console.log("\n=== Section 23: bulkUpdateCapacity Input Contracts ===")

function validateBulkCapacityInput(opts) {
  var errors = []
  if (!opts.startDate) errors.push("startDate required")
  if (!opts.endDate) errors.push("endDate required")
  if (opts.startDate && opts.endDate && opts.startDate > opts.endDate) errors.push("startDate must be <= endDate")
...
assertEqual(validateBulkCapacityInput({ startDate:"2026-04-01", endDate:"2026-04-30", capacity:5 }).length, 0, "bulkCapacity: valid passes")
assert(validateBulkCapacityInput({ startDate:"2026-04-01", endDate:"2026-04-30", capacity:0 }).includes("capacity must be >= 1"), "bulkCapacity: 0 rejected")
assert(validateBulkCapacityInput({ startDate:"2026-04-01", endDate:"2026-04-30", capacity:-1 }).includes("capacity must be >= 1"), "bulkCapacity: negative rejected")
assert(validateBulkCapacityInput({ startDate:"2026-04-01", endDate:"2026-04-30", capacity:5, slot:"BOGUS" }).includes("invalid slot filter"), "bulkCapacity: invalid slot rejected")

// =========================================================
// SECTION 24: getShiftsForRange — Return Shape Contract
// =========================================================
console.log("\n=== Section 24: getShiftsForRange Return Shape ===")

// Simulate the shape the new action returns for each shift
function validateShiftShape(shift) {
  var required = ["id","shift_date","slot","start_time","end_time","capacity","shift_assignments"]
  return required.every(function(k) { return k in shift })
}
function validateAssignmentShape(assignment) {
  var required = ["id","user_id","profiles"]
  return required.every(function(k) { return k in assignment })
}
function validateProfileShape(profile) {
  if (profile === null) return true // null is allowed
  var required = ["id","name","email"]
  return required.every(function(k) { return k in profile })
}

var sampleShift = {
  id: "s1", shift_date: "2026-04-15", slot: "AM", start_time: "09:00", end_time: "12:00", capacity: 3,
  shift_assignments: [{ id: "a1", user_id: "u1", profiles: { id: "u1", name: "Alice", email: "a@example.com" } }]
}
assert(validateShiftShape(sampleShift), "shift shape: all required keys present")
assert(Array.isArray(sampleShift.shift_assignments), "shift.shift_assignments is array")
assert(validateAssignmentShape(sampleShift.shift_assignments[0]), "assignment shape: all keys present")
assert(validateProfileShape(sampleShift.shift_assignments[0].profiles), "profile shape: all keys present")
// Null profiles (deleted user) should pass
assert(validateProfileShape(null), "null profile is valid (deleted user case)")
// Empty assignments array is valid
var emptyShift = { ...sampleShift, shift_assignments: [] }
assert(Array.isArray(emptyShift.shift_assignments) && emptyShift.shift_assignments.length === 0, "empty assignments array is valid")

// =========================================================
// SECTION 25: Admin Shift Grid — Week Range Computation
// =========================================================
console.log("\n=== Section 25: Admin Week Range Computation ===")
...
var wMar18 = getWeekRange(new Date(2026, 2, 18)) // Wed Mar 18
assertEqual(wMar18.start, "2026-03-15", "Mar 18 week starts Mar 15 (Sun)")
assertEqual(wMar18.end, "2026-03-21", "Mar 18 week ends Mar 21 (Sat)")

var wDec28 = getWeekRange(new Date(2026, 11, 28)) // Mon Dec 28
assertEqual(wDec28.start, "2026-12-27", "Dec 28 week starts Dec 27 (Sun)")
assertEqual(wDec28.end, "2027-01-02", "Dec 28 week ends Jan 2 (crosses year boundary)")

var wJan1 = getWeekRange(new Date(2027, 0, 1)) // Fri Jan 1
assertEqual(wJan1.start, "2026-12-27", "Jan 1 2027 week starts Dec 27 2026")
assertEqual(wJan1.end, "2027-01-02", "Jan 1 2027 week ends Jan 2 2027")
