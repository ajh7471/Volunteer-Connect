...
# Project root is always / in this environment
ROOT = "/"
...
# Step 1: Install vitest only if not already present
...
if not os.path.exists(vitest_bin):
    ...
    print("=== Vitest already installed, skipping npm install ===")

# Step 2: Run tests with verbose reporter
print("\n=== Running Tests ===")
config = os.path.join(ROOT, "vitest.config.ts")
r = run([vitest_bin, "run", "--config", config, "--reporter=verbose"], capture_output=False)
