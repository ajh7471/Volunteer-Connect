...
### Step 2a — 031a org_id backstop hotfix (Codex PR #36 finding P1)
`scripts/031a_org_id_backstop_triggers.sql` (merged via PR #37). `031`'s `org_id NOT NULL` broke app-layer inserts that never set `org_id` (SQLSTATE 23502). Added BEFORE INSERT triggers that auto-fill `org_id` only when the caller omits it (`backstop_org_id_from_shift` / `_from_user` / `_from_actor`; all `SECURITY DEFINER` + pinned `search_path`; fire `WHEN NEW.org_id IS NULL`), covering 10 tenant tables. The migration test grew to **9/9** (added shift/user/actor omitted-`org_id` cases).

🔴 **Phase 3 task — shifts residual (do NOT lose):** Phase 3 app-wiring: `bulkCreateShifts` / `createSingleShift` in `app/admin/actions.ts` insert into `shifts` via the SERVICE-ROLE client (no `auth.uid()`), so the 031a backstop does NOT cover `shifts`. These callers MUST pass `org_id` explicitly during Phase 3. Until then, admin shift creation will fail on a NOT-NULL `org_id` once real data exists.
